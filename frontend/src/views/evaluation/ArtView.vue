  ...
  <div class="page-view">
    ...
    <div class="page-card">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="artPerformance" label="美育成绩" width="100" align="center" />
        <el-table-column prop="artBonus" label="加分" width="70" align="center" />
        <el-table-column prop="artDeduction" label="扣分" width="70" align="center">
          <template #default="{ row }">
            <span style="color: #E57373">{{ row.artDeduction ?? '-' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑美育评价' : '新增美育评价'" width="520px" class="form-dialog" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <div class="form-grid">
          <el-form-item label="美育成绩"><el-input-number v-model="form.artPerformance" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="加分"><el-input-number v-model="form.artBonus" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="扣分"><el-input-number v-model="form.artDeduction" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
        </div>
...
import { getArtPage, saveArt, deleteArt } from '@/api/evaluation'
import { getStudentPage } from '@/api/student'
import type { ArtEvaluation, EvaluationQuery } from '@/types/evaluation'
...
async function loadData() {
  loading.value = true
  try {
    const res = await getArtPage(query) as unknown as { data: PageResult<ArtEvaluation> }
...
function openDialog(row?: ArtEvaluation) {
  isEdit.value = !!row
  if (row) {
    ...
    Object.assign(form, { id: undefined, studentId: undefined, academicYear: '', artPerformance: 0, artBonus: 0, artDeduction: 0, remark: '' })
  }
...
async function handleDelete(row: ArtEvaluation) {
  await ElMessageBox.confirm('确定删除该美育评价记录？', '提示', { type: 'warning' })
