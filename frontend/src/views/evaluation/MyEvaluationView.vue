<template>
  <div class="page-view">
    <div class="page-header">
      <h2 class="page-title">我的综合素质评价</h2>
    </div>
...
import { getMyComprehensive } from '@/api/evaluation'
...
onMounted(async () => {
  try {
    const res = await getMyComprehensive() as unknown as { data: ComprehensiveEvaluation[] }
    evaluations.value = Array.isArray(res.data) ? res.data : [res.data].filter(Boolean)
    if (evaluations.value.length) {
      selectedYear.value = evaluations.value[evaluations.value.length - 1].academicYear
    }
  } finally {
    loading.value = false
  }
