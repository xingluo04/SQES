  ...
  <div class="page-view">
    ...
    <div class="page-card">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="physicalPerformance" label="体育成绩" width="100" align="center" />
        <el-table-column prop="physicalBonus" label="加分" width="70" align="center" />
        <el-table-column prop="physicalDeduction" label="扣分" width="70" align="center">
          <template #default="{ row }">
            <span style="color: #E57373">{{ row.physicalDeduction ?? '-' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑体育评价' : '新增体育评价'" width="520px" class="form-dialog" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <div class="form-grid">
          <el-form-item label="体育成绩"><el-input-number v-model="form.physicalPerformance" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="加分"><el-input-number v-model="form.physicalBonus" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="扣分"><el-input-number v-model="form.physicalDeduction" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
        </div>
...
import { getPhysicalPage, savePhysical, deletePhysical } from '@/api/evaluation'
import { getStudentPage } from '@/api/student'
import type { PhysicalEvaluation, EvaluationQuery } from '@/types/evaluation'
...
const tableData = ref<PhysicalEvaluation[]>([])
const total = ref(0)
const formRef = ref<FormInstance>()

const query = reactive<EvaluationQuery>({ current: 1, size: 10, keyword: '', academicYear: '' })
const form = reactive<Partial<PhysicalEvaluation>>({})
...
async function loadData() {
  loading.value = true
  try {
    const res = await getPhysicalPage(query) as unknown as { data: PageResult<PhysicalEvaluation> }
...
function openDialog(row?: PhysicalEvaluation) {
  isEdit.value = !!row
  if (row) {
    ...
    Object.assign(form, { id: undefined, studentId: undefined, academicYear: '', physicalPerformance: 0, physicalBonus: 0, physicalDeduction: 0, remark: '' })
  }
...
async function handleDelete(row: PhysicalEvaluation) {
  await ElMessageBox.confirm('确定删除该体育评价记录？', '提示', { type: 'warning' })
  await deletePhysical(row.id!)
