  ...
  <div class="page-view">
    ...
    <div class="page-card">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="laborPerformance" label="劳动成绩" width="100" align="center" />
        <el-table-column prop="laborBonus" label="加分" width="70" align="center" />
        <el-table-column prop="laborDeduction" label="扣分" width="70" align="center">
          <template #default="{ row }">
            <span style="color: #E57373">{{ row.laborDeduction ?? '-' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑劳动评价' : '新增劳动评价'" width="520px" class="form-dialog" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <div class="form-grid">
          <el-form-item label="劳动成绩"><el-input-number v-model="form.laborPerformance" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="加分"><el-input-number v-model="form.laborBonus" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
          <el-form-item label="扣分"><el-input-number v-model="form.laborDeduction" :min="0" :max="100" :step="0.5" controls-position="right" /></el-form-item>
        </div>
...
import { getPracticePage, savePractice, deletePractice } from '@/api/evaluation'
import { getStudentPage } from '@/api/student'
import type { PracticeEvaluation, EvaluationQuery } from '@/types/evaluation'
...
const tableData = ref<PracticeEvaluation[]>([])
const total = ref(0)
const formRef = ref<FormInstance>()

const query = reactive<EvaluationQuery>({ current: 1, size: 10, keyword: '', academicYear: '' })
const form = reactive<Partial<PracticeEvaluation>>({})
...
async function loadData() {
  loading.value = true
  try {
    const res = await getPracticePage(query) as unknown as { data: PageResult<PracticeEvaluation> }
...
function openDialog(row?: PracticeEvaluation) {
  isEdit.value = !!row
  if (row) {
    ...
    Object.assign(form, { id: undefined, studentId: undefined, academicYear: '', laborPerformance: 0, laborBonus: 0, laborDeduction: 0, remark: '' })
  }
...
async function handleDelete(row: PracticeEvaluation) {
  await ElMessageBox.confirm('确定删除该劳动评价记录？', '提示', { type: 'warning' })
