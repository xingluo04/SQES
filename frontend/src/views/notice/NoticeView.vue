      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="type" label="类型" width="90">
          <template #default="{ row }">
            <el-tag :type="row.type === 1 ? 'warning' : row.type === 2 ? 'success' : 'info'" size="small">
              {{ row.type === 1 ? '紧急' : row.type === 2 ? '喜报' : '普通' }}
            </el-tag>
          </template>
        </el-table-column>
...
import { getNoticePage, createNotice, updateNotice, deleteNotice } from '@/api/notice'
...
async function loadData() {
  loading.value = true
  try {
    const res = await getNoticePage(query) as unknown as { data: PageResult<Notice> }
